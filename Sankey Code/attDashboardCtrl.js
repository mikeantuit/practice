'use strict'
app.controller('attDashboardController', function ($stateParams, $scope, $state, $http, $q) {
    var vm = this;
    var date = [],
    ...
    d3.csv('attribution.csv', function (error, originData) {
        ...
        var chartDataPie = getChartDataPie(copy2)
        var chartDataPieDrillDown = getChartDataPieDrillDown(copy);
        var chartDataLine = getLineData(copy3);
        drawPieChart(chartDataPie, chartDataPieDrillDown);
        drawLineChart1(chartDataLine, copy4);
        var copy2 = angular.copy(originData);
        var chartDataLineDrillDown = getLineDrillDownData(copy2, 'internet')
        drawLineChartDrillDown1(chartDataLineDrillDown, "internet")
    ...
    function getChartDataPieDrillDown(data) {
        ...
        angular.forEach(uniqueChannels, function (value, key) {
            ...
            var uniqueChannelsData = _.filter(data, {
            ...
            for (var i = 0; i < uniqueChannelsData.length; i++) {
                ...
                uniqueChannelsData[i]['percentage'] = uniqueChannelsData[i]['percentage'] / sum * 100;
                result[key]["data"].push([uniqueChannelsData[i]['subchannel1'], uniqueChannelsData[i]['percentage']]);
            }
        })
        return result;
    }
    ...
    function drawPieChart(data, data1) {
        console.log(data)
        console.log(data1)
        vm.pieChartAtt = Highcharts.chart('piechart', {
            credits: {
                enabled: false
            },
            chart: {
    ...
    function drawLineChart1(data, originData) {
        vm.lineChartAtt = Highcharts.StockChart('linechart', {
            credits: {
                enabled: false
            },
            chart: {
    ...
    function drawLineChartDrillDown1(data, name) {
        vm.lineChartAttDrillDown = Highcharts.StockChart('sublinechart', {
            credits: {
                enabled: false
            },
            chart: {
