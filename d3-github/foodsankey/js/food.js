...
function disp_content(a,hash){
	...
	d3.select("#map").transition().delay(250).style("visibility", "hidden");
	d3.select("#scatter").transition().delay(250).style("visibility", "hidden");
	d3.select("#description").transition().delay(250).style("visibility", "hidden");
	//set title
	...
	if (( event.keyCode == 27 ) || ( event.keyCode >= 33 && event.keyCode <= 34 ) || (event.keyCode >= 37 && event.keyCode <= 40)) {
		switch( event.keyCode ) {
			...
			case 38: // up
					 ...
					 else if (d3.select("#scatter").style("opacity")==1) {scrollscatter(1)}; //scroll if on the scatter page
					 break;
			case 34: // pg down
			case 39: // right
			case 40: // down 
					 if (d3.select("#content").style("opacity")==1) {scrollsankey(-1)}
					 else if (d3.select("#scatter").style("opacity")==1) {scrollscatter(-1)};
					 break;
		}
		...
		d3.json(datapath+"json/"+dropdown.node().value+"m.json", function(d) {
			...
			d3.select("#cid").text(dropdown.node().value)
				...
				change = function() {
					...
					var eroei_time_graph = function(d,e,f) {
					...
					eroei_time_graph(eroei_raw[a3],aec[a3],eroei[a3]);
					...
					var data_quality_info=function(){
						...
						var qualitytooltip="<table style='font-size:12px;'><tr><td style='border-bottom:solid 1px #888;'>Data availability for <b>"+yearselect.node().value+": </b>";
